...
ui(:,1,1) = two.*u(:,2,1)-u(:,3,1);
ui(:,imax,1) = two.*(:,imax-1,1) - u(:,imax-2,1);
...
for xcoord = 2:imax - 1
    for ycoord = 2:jmax - 1
        ...
        % Pressure
        res_p(ycoord,xcoord) = (rho * (u(ycoord, xcoord+1,2)-u(ycoord,xcoord-1,2)))/(two*dx) + ...
        (rho*(u(ycoord+1, xcoord, 3)-u(ycoord-1,xcoord,3)))/(two*dy) - ...
        (artviscx(ycoord,xcoord)+artviscy(ycoord,xcoord)) - s(ycoord, xcoord);
